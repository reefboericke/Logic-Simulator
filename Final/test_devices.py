...
def test_make_device(new_devices):
    ...
    [NAND1_ID, CLOCK1_ID, SIGGEN1_ID, D1_ID, I1_ID,
     I2_ID] = names.lookup(["Nand1", "Clock1", "Siggen1", "D1", "I1", "I2"])
    ...
    new_devices.make_device(SIGGEN1_ID, new_devices.SIGGEN, '1101001')
    ...
    # Siggen waveform starts with '1' (= high)
    assert siggen_device.outputs == {None: new_devices.HIGH}
    ...
    # Siggen counter initially at 0
    assert siggen_device.clock_counter == 0
...
def test_make_device_gives_errors(new_devices, function_args, error):
    ...
    names = new_devices.names
    [AND1_ID, SW1_ID, CL_ID, SG_ID, D_ID, X1_ID,
     X2_ID] = names.lookup(["And1", "Sw1", "Clock1", "Siggen1", "D1",
                            "Xor1", "Xor2"])
